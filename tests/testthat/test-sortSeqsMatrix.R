...
test_that("Test standard cases", {
  #Create arguments for sortSeqsMatrix
  ...
  #Sort matrix
  matSeqs <- sortSeqsMatrix(matSeqs, TRUE)
  ...
  #Sort matrix
  matSeqs <- sortSeqsMatrix(matSeqs, FALSE)
...
test_that("Test with order.regions", {
  ...
  #Sort matrix
  matSeqs <- sortSeqsMatrix(matSeqs, FALSE, order.regions = TRUE)
...
test_that("Test on parameters type", {
  ...
  expect_error(sortSeqsMatrix(4, TRUE),
               "This function require that seqsMat is a matrix of numerics")
  ...
  expect_error(sortSeqsMatrix(mat, TRUE),
               "This function require that seqsMat is a matrix of numerics")
  
  #check if get error when reduced isn't logical
  expect_error(
    sortSeqsMatrix(mat_check, mat_check),"decreasing must be logical!")
  
  #check if get error when reduced isn't logical
  expect_error(
    sortSeqsMatrix(mat_check, TRUE, order.regions = "4"),
    "order.regions must be logical!")
})